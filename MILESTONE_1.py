"""
AI Code Reviewer v6 - with Google/NumPy/ReST docstring style selector + AI Fix (Groq)
Run: pip install streamlit radon plotly requests && streamlit run ai_code_reviewer_pro.py
...
with st.sidebar:
    ...
    view_options=["Home","Dashboard","Metrics","Docstring Coverage","Validation","Summary Report"]
    def _on_nav_change():
...
""",unsafe_allow_html=True)
    ...
    if view=="Metrics":
        ...
        pep=st.session_state.pep257_issues; total_pep=sum(len(v) for v in pep.values())
        high_cx=len(df[df["complexity_level"]=="High"]); avg_mi=df["maintainability_index"].mean()
        doc_pct=round(len(df[df["docstring_status"]=="Documented"])/len(df)*100)
        st.markdown('<div style="font-family:\'Share Tech Mono\',monospace;font-size:.6rem;color:#007a8a;letter-spacing:.2em;text-transform:uppercase;margin-bottom:4px;">[ REPORT ]</div><h2 style="font-family:\'Oxanium\',sans-serif;font-size:1.6rem;font-weight:800;margin:0 0 20px;color:#cdd9f5;letter-spacing:.04em;">Summary Report</h2>',unsafe_allow_html=True)
        c1,c2,c3,c4=st.columns(4)
        c1.metric("Avg MI Score",f"{avg_mi:.1f}"); c2.metric("Doc Coverage",f"{doc_pct}%")
        c3.metric("PEP 257 Issues",total_pep); c4.metric("High Complexity",high_cx)
        st.markdown("---")
        tab_t,tab_j=st.tabs(["📊 Table","{ } JSON"])
        with tab_t: st.dataframe(df.drop(columns=["file_path","_source"],errors="ignore"),use_container_width=True,hide_index=True)
        with tab_j:
            summary_json={"scan_number":st.session_state.scan_count,"avg_maintainability_index":round(float(avg_mi),2),
                          "docstring_coverage_pct":doc_pct,"pep257_total_issues":total_pep,"high_complexity_functions":high_cx,"per_file":{}}
            for fname,group in df.groupby("file_name"):
                summary_json["per_file"][fname]={"functions":group.drop(columns=["file_path","file_name","_source"],errors="ignore").to_dict(orient="records"),"pep257_issues":pep.get(fname,[])}
            json_str=json.dumps(summary_json,indent=2)
            st.markdown(f'<div class="json-viewer">{json_str}</div>',unsafe_allow_html=True)
            st.download_button("📥 Download Full Report JSON",json_str,file_name="full_report.json",mime="application/json")